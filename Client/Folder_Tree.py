...
# PyG Data 객체는 "노드의 특성 행렬", "노드간의 연결 정보", "그래프 전체 라벨" 총 3개의 값 필요요


def build_graph_from_folder(root_path, label: int = 0, with_label: bool = True):
    G = nx.DiGraph()
        # Networkx 라이브러리의 "유향 그래프"(Directed Graph) 객체인 "G"를 생성
    node_idx = {}
    idx = 0
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):   
        ...
        for dname in dirnames:
            ...
            node_idx[full_path] = idx                   # "폴더의 전체 경로 : index 값" 의 형태로 Key:Value 저장
        ...
        for fname in filenames:
            ...
            node_idx[full_path] = idx                   # "파일의 전체 경로 : index 값" 의 형태로 Key:Value 저장
            is_file = 1             # 1 = 파일
            try:    # 파일 크기 추출 시도
        ...
        for dname in dirnames:          
            ...
            if p in node_idx and c in node_idx:        # 부모, 자식에 대한 노드가 존재한다면,
                G.add_edge(node_idx[p], node_idx[c])   # 두 노드 사이의 방향 간선을 G에 추가
                    # edge_index 정보 추가
